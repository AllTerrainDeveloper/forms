 ...
 *
 * Listed here rather than read from the plugin's own code, because the plugin's
 * files are not loaded during uninstall and requiring them would run its
 * bootstrap in a context it was never written for.
 */
$atf_caps = array(
	'atf_edit_forms',
	'atf_read_entries',
...
if ( is_dir( $atf_dir ) ) {
	...
	foreach ( $atf_files as $atf_file ) {
		if ( $atf_file->isDir() ) {
			@rmdir( $atf_file->getRealPath() ); // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged, WordPress.WP.AlternativeFunctions.directory_rmdir -- Best effort during uninstall; a permissions failure must not fatal the deletion.
			continue;
		}
	...
	@rmdir( $atf_dir ); // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged, WordPress.WP.AlternativeFunctions.directory_rmdir -- As above.
}

 ...
 * result was stashed on `wp` by `alltfo_handle_post_submission()`; here it is
...
add_shortcode( 'allterrain_form', 'alltfo_shortcode' );
 ...
 * @return string
 */
function alltfo_shortcode( $atts ) {
	$atts = shortcode_atts(
		array(
			'id'    => 0,
			'theme' => '',
	...
	$stashed = alltfo_stash_result( $form_id );
	...
	if ( $stashed ) {
		...
		if ( $result['success'] ) {
			...
			// `alltfo_handle_post_submission()`; reaching here means it was a
			...
			$args['values'] = alltfo_rehydrate_values( $form_id, $stashed['request'] );
			...
			if ( '' !== $result['message'] && ! $result['errors'] ) {
				...
				return sprintf(
					...
					alltfo_render_form( $form_id, $args )
				);
			}
		}
	}

	return alltfo_render_form( $form_id, $args );
}
 ...
 */
function alltfo_rehydrate_values( $form_id, $raw ) {
	if ( ! is_array( $raw ) ) {
		return array();
	}

	return alltfo_sanitize_submission( alltfo_get_form_schema( $form_id ), $raw );
}
